...
- Papers (all under review, 2026; never name venues): "How Valid are Metrics of Chain of Thought Faithfulness with LLM Judges?" (a placebo cue reproduces most of the measured cue influence in small models); "What Happens to a Monitor's Accuracy When You Train Against It" (with Anagha Ramaswamy; a correctness probe used as an RL reward drops true accuracy ~31 points while its own accuracy lags); "How Small Can We Go? Calibrating Activation-Cache Compression for SAE Training"; "Coding Agents for Coding Theory" (an LLM coding agent under a verifier-first protocol improved published lower bounds for quaternary edit-metric codes). Summaries and PDFs: abrahamyeung.com/papers
- Projects: gigabpe (Rust BPE tokenizer trainer with Python bindings, on PyPI; 6.8x faster than HuggingFace tokenizers on a 12.9 GB FineWeb run with byte-identical merges); a Transformer language model built from scratch in CS 336 (Triton FlashAttention, DDP, SFT and GRPO); Research Frontier Miner (autonomous research-idea agent, in progress, no results yet); Prediction Markets Agent (top 5 of 100+ at the NVIDIA/Vercel/Brex hackathon); multi-agent causal modeling at the Bridgewater AI Hackathon (1 of 24 selected); Maestro (multimodal music coach, TreeHacks music track runner-up); AQI forecasting with LSTM/GNN/CNN (CS 229).
...
- Leadership: YC Fellow, summer 2026 (mentored by Harshita Arora); TreeHacks Summer Fellow (2026); former Director of Hackspace at BASES (Sep 2024 - Jun 2026); baritone and Social Chair in the Stanford Mendicants a cappella group.
